...
public class ShootingController : MonoBehaviour
{
    ...
    [Tooltip("The instant kill projectile to be fired.")]
    public GameObject instakillPrefab = null;
    ...
    public void ApplyFireRateBoost(float amount, float duration)
    {
        // Cancel any ongoing coroutine (no undo needed)
        ...
        {
            // Apply the boost only once
            fireRate -= amount;
        }

        fireRateBoostRoutine = StartCoroutine(RemoveFireRateBoost(amount, duration));
    }

    private IEnumerator RemoveFireRateBoost(float amount, float duration)
    {
    ...
    public void ApplyInstaKill(float duration)
    {
        if (instakillPrefab == null) return;
        ...
        {
            // Swap to instakill only once
            GameObject temp = projectilePrefab;
            projectilePrefab = instakillPrefab;
            instakillPrefab = temp;
        }

        instaKillRoutine = StartCoroutine(RemoveInstaKill(duration));
    }

    private IEnumerator RemoveInstaKill(float duration)
    {
        float elapsedTime = 0f;
        while (elapsedTime < duration)
        {
