...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Tooltip("Maximum number of enemies allowed alive at the same time.")]
    public int maxEnemiesAlive = 5;

    // Track currently alive enemies
    ...
    {
        if (enemyPrefab != null && aliveEnemies.Count < maxEnemiesAlive)
        {
            ...
            // Track spawned enemy
            aliveEnemies.Add(enemyGameObject);
            currentlySpawned++;
            lastSpawnTime = Time.timeSinceLevelLoad;

            // Optional: Remove from list on death (see below)
            enemyGameObject.GetComponent<Health>().OnDeath += () =>
            {
                aliveEnemies.Remove(enemyGameObject);
            };
        }
    }
