...
public class Enemy : MonoBehaviour
{
    ...
    void OnTriggerStay2D(Collider2D other)
    {
        ...
        {
            Debug.Log("Enemy touched a wall trigger!");
            Vector3 wallToEnemy = transform.position - other.transform.position;
        
            // Manually reposition enemy back from wall
            transform.position += wallToEnemy.normalized * moveSpeed * 5 * Time.deltaTime;
        }
    }
