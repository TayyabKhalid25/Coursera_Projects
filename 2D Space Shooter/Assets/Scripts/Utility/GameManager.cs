...
public class GameManager : MonoBehaviour
{
    ...
    public void BossDefeated()
    {
        ...
        if (UIManager.instance != null)
        {
            player.GetComponent<Health>().isAlwaysInvincible = true; // Make sure the player is invincible
            UIManager.instance.allowPause = false;
            if (victoryEffect != null)
            {
                Instantiate(victoryEffect, transform.position, transform.rotation, null);
    ...
    private IEnumerator WaitForBossDefeat()
    {
        ...
        UIManager.instance.GoToPage(gameVictoryPageIndex);
        player.SetActive(false); // Disable the player after the victory effect
    }
