...
public class Enemy : MonoBehaviour
{
    ...
    [Tooltip("The maximum distance from the target that this enemy will begin to move toward it")]
    public float maximumMoveRange = 5.0f;
    ...
    [Tooltip("Whether or not this enemy needs line of sight of target to move")]
    public bool lineOfSightToMove = true;
    [Tooltip("Whether or not this enemy needs line of sight to target to attack")]
    public bool lineOfSightToAttack = true;
    [Tooltip("The layers that can be hit when checking line of sight")]
    public LayerMask lineOfSightHitLayers;
    ...
    {
        ActiveEnemies.Add(this); // I just spawned, add me to the list
    }

    private void OnDisable()
    {
        ActiveEnemies.Remove(this); // I died/disabled, remove me
    }

    private void OnDestroy()
    {
        GameManager.instance.AddKill(); // I died/disabled, increment kill count
    }
    ...
    protected virtual void HandleMovement()
    {
        ...
        if (canMove && enemyRigidbody != null && attackMove && hasLineOfSight && (target.position - transform.position).magnitude < maximumMoveRange)
        {
    ...
    protected virtual bool HasLineOfSight()
    {
        if (target != null)
        {
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, lineOfSightHitLayers))
            {
