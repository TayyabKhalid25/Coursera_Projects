...
public class Health : MonoBehaviour
{
    ...
    void Die()
    {
        ...
        if (enemy != null)
        {
            // Add score using the existing GameManager logic
            GameManager.AddScore(enemy.scoreOnKill);
            if (enemy.keyToDrop != null)
            {
                Vector3 offset = Vector3.zero;
                if(gameObject.tag == "Boss")
                {
                    offset = Vector3.down * 3;
                }
                Instantiate(enemy.keyToDrop, transform.position + offset, transform.rotation, null);
            }
        }
